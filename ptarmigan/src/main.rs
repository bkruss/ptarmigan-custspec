...
struct CollideOptions {
    ...
    classical: bool,
    // Use particle tracking for ELECTRON_MASS
    track_electrons: bool,
    // Index of first tracked particle
    tracked_index: u64,
...
fn collide<F: Field, R: Rng>(field: &F, incident: Particle, rng: &mut R, current_id: &mut u64, options: CollideOptions) -> Shower {
    ...
    let mut tracked: Vec<Particle> = Vec::new();
    ...
    let save_primary: bool = if options.track_electrons && (primary_id % options.tracked_index == 0) {
        true
    } else {
        false
    };
        ...
        match pt.species() {
            Species::Electron | Species::Positron => {
                while field.contains(pt.position()) && pt.time() < options.t_stop {
                    ...
                    if save_primary && pt.id() == primary_id {
                        tracked.push(pt);
                    }
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let track_electrons = input.read("control:track_electrons").unwrap_or(false);

    let tracked_fraction: f64 = input.read("control:tracked_fraction").unwrap_or(0.0);
    ...
    let use_raw_particles = input.read("beam:raw_particles").unwrap_or(false);
    ...
    let raw_file = if use_raw_particles {
        input.read::<String,_>("beam:raw_file")?
    } else {
        "N/A".to_string()
    };
    ...
    let mut raw_x: Vec<f64>;
    let mut raw_y: Vec<f64>;
    let mut raw_z: Vec<f64>;
    let mut raw_px: Vec<f64>;
    let mut raw_py: Vec<f64>;
    let mut raw_pz: Vec<f64>;
    let mut raw_weight: Vec<f64>;
    let start_ind: Vec<usize>;


    let (raw_x, raw_y, raw_z, raw_px, raw_py, raw_pz, raw_weight, num, start_ind) = if use_raw_particles {
        use std::fs::File;

        let file_path = Path::new(&raw_file);
        ...
        for result in rdr.records() {
            ...
            raw_x.push(record[0].parse().unwrap());
            raw_y.push(record[1].parse().unwrap());
            raw_z.push(record[2].parse().unwrap());
            raw_px.push(record[3].parse().unwrap());
            raw_py.push(record[4].parse().unwrap());
            raw_pz.push(record[5].parse().unwrap());
            raw_weight.push(record[6].parse().unwrap());
        }

        let npart_raw: usize = raw_x.len() as usize;

        let nums: Vec<usize> = {
            let tasks = ntasks as usize;
            (0..tasks).map(|i| (npart_raw * (i + 1) / tasks) - (npart_raw * i / tasks)).collect()
        };
        assert_eq!(nums.iter().sum::<usize>(), npart_raw);
        let num = nums[id as usize];

        let mut start_ind: Vec<usize> = Vec::new();

        for (i,_n) in nums.iter().enumerate() {
            if i == 0 {
                start_ind.push(0)
            } else if i < nums.len() {
                start_ind.push(start_ind[i-1] + nums[i-1]);
            }
        }
//        println!("nums = {:?}, start_ind = {:?}", nums, start_ind);
        (raw_x, raw_y, raw_z, raw_px, raw_py, raw_pz, raw_weight, num, start_ind)
...
// Add spectra here
    ...
    } else if use_raw_particles {
        builder.with_raw_particles()
    }
    else {
        builder.with_normal_energy_spectrum(gamma, sigma)
    };

    let primaries = builder.build(&mut rng, spectrum_file, id, raw_x, raw_y, raw_z, raw_px, raw_py, raw_pz, raw_weight, start_ind);

    let mut current_id = num as u64;

    let merge = |(mut e, mut g, mut p, mut trk): (Vec<Particle>, Vec<Particle>, Vec<Particle>, Vec<Particle>), mut sh: Shower| {
        ...
        while let Some(pt) = sh.tracked.pop() {
            trk.push(pt);
        }
        (e, g, p, trk)
    };

    let tracked_index: u64 = if track_electrons {
        let t_ind = 1.0/tracked_fraction;
        t_ind as u64
    } else{
        0
    };
    ...
    let (mut electrons, mut photons, mut positrons, mut tracked_e) = primaries
        .chunks(num / 20)
        .enumerate()
        .map(|(i, chk)| {
            let tmp = chk.iter()
    ...
    for pt in electrons.iter_mut().chain(photons.iter_mut()).chain(positrons.iter_mut()).chain(tracked_e.iter_mut()) {
    ...
    match output_mode {
        ...
        OutputMode::Hdf5 => {
            ...
            conf.new_group("control")?
                ...
                .new_dataset("track_electrons")?.write(&track_electrons)?
                .new_dataset("tracked_fraction")?.write(&tracked_fraction)?
            ...
            conf.new_group("beam")?
                ...
                .new_dataset("custom_spectrum")?.write(&use_custom_spectrum)?
